...
public class MethodRef {
     ...
     */
        ...
        // 3) 메서드 참조 방식으로 변환( :: )
        // 람다식 num -> Math.abs(num) 이랑 같은 모양, 같은 역할의 메서드
        ...
        Supplier<String> lower3 = str::toLowerCase;
                                // str 객체의 toLowerCase 메서드를 사용하겠다.
        ...
        // 2) 람다식
        Function<String,Integer> strLength2 = s -> str.length();
        System.out.println("str 길이 2 : "+strLength2.apply(str));

        // 3) 메서드 참조 방식
        Function<String,Integer> strLength3 = String::length;
        System.out.println("str 길이 3 : "+strLength3.apply(str));

        System.out.println("----------------------------------------");


        // 리스트 순차 접근 후 출력
        // List.of() : 값을 바로 넣어서 리스트를 만드는 방법 -> 수정이 불가능함(immutable)
        List<String> list = List.of("a", "b", "c", "d", "e", "f", "g", "h");
//        list.add("E");           --> 추가,삭제 등 수정 불가능
//        list.remove("a");
       ...
       /* for(String s : list){
        ...
        // 2) 람다식
        Consumer<String> pringList2 = s -> System.out.println(s);
        System.out.println("printList2 : " + list);
        list.forEach(pringList2);

        // 3) 메서드 참조 방식
        System.out.println("printList3 : " + list);
        ...
        // 3) 매개변수 메서드 참조 방식
        System.out.println("sum3 : "+sumTwoNumber(MethodRef::sum1,10,20));
